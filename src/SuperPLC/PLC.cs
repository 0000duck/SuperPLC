...
namespace S7.Net
{
    ...
    public class Plc : IDisposable
    {
                    ...
                    case "DB":
                        ...
                        switch (dbType)
                        {
                            ...
                            case "DBD":
                                ...
                                return objU;
                            case "DBR":
                                return float.Parse(Read(DataType.DataBlock, mDB, dbIndex, VarType.Real, 1).ToString());
                            case "DBS":
                                return Read(DataType.DataBlock, mDB, dbIndex, VarType.String, 10).ToString();
                            case "DBL":
                                byte[] bytes = ReadBytes(DataType.DataBlock, mDB, dbIndex, 8);
                ...
                switch (txt.Substring(0, 2))
                {
                    case "DB":
                        ...
                        switch (dbType)
                        {
                            ...
                            case "DBR":
                                objValue = Convert.ChangeType(value, typeof(float));
                                return Write(DataType.DataBlock, mDB, dbIndex, (float)objValue);
                            case "DBL":
                                return WriteBytes(DataType.DataBlock, mDB, dbIndex, LongToBytes(long.Parse(value.ToString())));
                            case "DBX":
                                mByte = dbIndex;
                                mBit = int.Parse(strings[2]);
                                if (mBit > 7)
        ...
        /// <returns></returns>
        private byte[] LongToBytes(long date)
        {
            byte[] bytes = new byte[8];
            for (int i = 0; i < 8; i++)
            {
                bytes[i] = (byte)(date >> (56 - (i * 8)));
            }
            return bytes;
        }
        ...
        /// <summary>
        /// Releases all resources, disconnects from the plc and closes the socket
        /// </summary>
        public void Dispose()
        {
            if (_mSocket != null)
